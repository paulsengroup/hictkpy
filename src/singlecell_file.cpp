...
static std::string repr(const SingleCellFile& scf) {
  return fmt::format(FMT_STRING("SingleCellFile({})"), scf->path());
}
...
void SingleCellFile::bind(nb::module_& m) {
  ...
  scell_file.def(
      "__enter__", [](SingleCellFile& scf) -> SingleCellFile& { return scf; },
      nb::rv_policy::reference_internal);

  scell_file.def(
      "__exit__",
      [](SingleCellFile& scf, [[maybe_unused]] nb::handle exc_type,
         [[maybe_unused]] nb::handle exc_value,
         [[maybe_unused]] nb::handle traceback) { scf.try_close(); },
      // clang-format off
  ...
  scell_file.def(
      "path", [](const SingleCellFile& scf) -> std::filesystem::path { return scf->path(); },
      "Get the file path.", nb::rv_policy::move);

  scell_file.def("close", &SingleCellFile::close, "Manually close the file handle.");

  scell_file.def(
      "resolution",
      [](const SingleCellFile& scf) { return static_cast<std::int64_t>(scf->resolution()); },
      "Get the bin size in bp.");
  scell_file.def(
      "chromosomes",
      [](const SingleCellFile& scf, bool include_ALL) {
        return get_chromosomes_from_object(*scf, include_ALL);
      },
      nb::arg("include_ALL") = false,
      "Get the chromosome sizes as a dictionary mapping names to sizes.",
      nb::rv_policy::take_ownership);
  scell_file.def(
      "bins", [](const SingleCellFile& scf) { return get_bins_from_object(*scf); },
      nb::sig("def bins(self) -> hictkpy.BinTable"), "Get table of bins.", nb::rv_policy::move);
