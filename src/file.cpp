...
void File::bind(nb::module_ &m) {
  ...
  file.def("__enter__", [](File &f) -> File & { return f; }, nb::rv_policy::reference_internal);

  file.def(
      "__exit__",
      [](File &f, [[maybe_unused]] nb::handle exc_type, [[maybe_unused]] nb::handle exc_value,
         [[maybe_unused]] nb::handle traceback) { f.try_close(); },
      // clang-format off
  ...
  file.def(
      "uri", [](const File &f) { return f->uri(); }, "Return the file URI.", nb::rv_policy::move);
  file.def(
      "path", [](const File &f) -> std::filesystem::path { return f->path(); },
      "Return the file path.", nb::rv_policy::move);

  file.def(
      "is_hic", [](const File &f) { return f->is_hic(); }, "Test whether file is in .hic format.");
  file.def(
      "is_cooler", [](const File &f) { return f->is_cooler(); },
  ...
  file.def(
      "chromosomes",
      [](const File &f, bool include_ALL) { return get_chromosomes_from_object(*f, include_ALL); },
      nb::arg("include_ALL") = false,
      "Get chromosome sizes as a dictionary mapping names to sizes.",
      nb::rv_policy::take_ownership);
  file.def(
      "bins", [](const File &f) { return get_bins_from_object(*f); }, "Get table of bins.",
      nb::sig("def bins(self) -> hictkpy.BinTable"), nb::rv_policy::move);

  file.def("resolution", [](const File &f) { return f->resolution(); }, "Get the bin size in bp.");
  file.def("nbins", [](const File &f) { return f->nbins(); }, "Get the total number of bins.");
  file.def(
      "nchroms", [](const File &f, bool include_ALL) { return f->nchroms(include_ALL); },
      nb::arg("include_ALL") = false, "Get the total number of chromosomes.");
  ...
  file.def(
      "has_normalization",
      [](const File &f, std::string_view norm) { return f->has_normalization(norm); },
      nb::arg("normalization"), "Check whether a given normalization is available.");
